...
feux_pasto<-read_excel("data/Base-feux-agricoles-pastoraux2001-2021-10092021.xlsx",sheet = 2)
...
```{r}
feux_pasto<-as_tibble(feux_pasto)
feux_pasto%>%
  filter(!mois=="")%>%
  select(!rmq&!`...10`)->feux_pasto
```
...
tm_shape(feux_sf)+
  tm_dots(size="surface_ha",col="surface_ha",palette=c('#ffffcc','#ffeda0','#fed976','#feb24c','#fd8d3c','#fc4e2a','#e31a1c','#b10026'),
          title="Burned area (ha)",alpha=0.8,n=8,scale=2)->Burned_area_bubbles
print(Burned_area_bubbles)
tm_shape(feux_sf)+
...
```{r}
tmap_save(Burned_area_bubbles, filename = "output_data/Burned_area_bubbles.html")
```
